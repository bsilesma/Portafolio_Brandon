...
public class CarritoController {
    ...
    // --- 2. AGREGAR PRODUCTO AL CARRITO (por AJAX, devuelve un fragmento) ---
    ...
    public String modificar(
        ...
        if (item == null) {
            // El ítem no está en el carrito: se regresa al listado
            return "redirect:/carrito/listado";
        }
    ...
    public String verFactura(@ModelAttribute("idFactura") Integer idFactura, Model model) {
        if (idFactura == null) {
            // Si no se pasó el ID por flash, se regresa a la tienda
            return "redirect:/";
        }

        // Obtener la factura COMPLETA (incluyendo ventas)
